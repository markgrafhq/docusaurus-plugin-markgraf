...
const CanvasPlayer = ({
  ...
  className,
  controls,
}: MarkgrafProps & { theme: Theme; controls: MarkgrafControls }) => {
...
const MountedPlayer = (props: MarkgrafProps) => {
  const theme = useDocumentTheme();
  return props.renderer === "svg" ? (
    <SvgPlayer
      {...props}
      renderer="svg"
      theme={theme}
...
export default function Markgraf(props: MarkgrafProps) {
  ...
  if (!mounted) {
    return (
      <div
        className={`markgraf-doc-player ${props.className ?? ""}`}
        data-markgraf-placeholder=""
      />
    );
  }

  return <MountedPlayer {...props} />;
}
